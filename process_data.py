...
Original file is located at
    https://colab.research.google.com/drive/1YWeJ8s1fSQxCr8vWHzHQY7sMtNfDlUaY
"""

# from google.colab import drive
# drive.mount('/content/drive/')

# %cd /content/drive/MyDrive/relation-extraction-vn
...
# split_entity('<e1>cư dân</e1> đang di chuyển về <e2>trung tâm thành phố</e2>')

# dependency_parsing_edge('Một nhóm sinh viên từ Đại học Công nghệ Virginia đã tạo ra một phương tiện cho phép ^người lái xe^ khiếm thị điều khiển ^bánh xe^')

# # sentence_bump(['<e1>cư dân</e1> đang di chuyển về <e2>trung tâm thành phố</e2>'])
# #split_entity('<e1>cư dân</e1> đang di chuyển về <e2>trung tâm thành phố</e2>')
# # get_feature(['Một nhóm sinh viên từ Đại học Công nghệ Virginia đã tạo ra một phương tiện cho phép <e1>người lái xe</e1> khiếm thị điều khiển <e2>bánh xe</e2>'
# # ])
# get_feature(['<e1>cư dân</e1> đang di chuyển về <e2>thành phố</e2>'])

def split_entity_and_create_edges(s):
    #tokenize and create edge on the original sentence (remain e mark)
    edges=dependency_parsing_edge(s)
    s=''
    precessed_edges=[]
    mapping={-1: -1}
    k=0
    ...
    #split entity
    try:
      sentence="" # is 's' but dont have e mark
      epos=[]
      seps=['^', '^', '^', '^']
      ...
      return {'sentence': sentence, 'epos': epos, 'token': token, 'edges': precessed_edges}
...
def dependency_parsing_edge(sentence):
    # doc = nlp(sentence)
    edges = dependency_parse(sentence) #nv
    edges_processed=[]
    # splitted=[]
    for i, edge in enumerate(edges):
        #edge: ('cư dân', 3, 'nsubj')
        edges_processed.append([i, edge[1]-1, edge[2], edge[0]])
        #token pos, token's head pos, their dependency type, token text
...
def sentence_bump(data):
    if isinstance(data, list):
        data = pd.DataFrame({'sentence': data})

    regex=''

    # print(data['sentence'].iloc[0])
    data['sentence']=data['sentence'].apply(lambda x: ' '.join(re.findall('<e1>*|<\/e1>*|<e2>*|<\/e2>*|\w*', x)))
    data['sentence']=data['sentence'].apply(lambda x: re.sub(r'<e1>|<\/e1>|<e2>|<\/e2>', '^', x))
    # print(data['sentence'].iloc[0])
    # data['sentence']=data['sentence'].apply(lambda x: re.sub(r'người\s+lái\s+xe', 'người_lái_xe', x))
    # print('alo')
    # print(data['sentence'])
    data2=list(data.apply(lambda row: split_entity_and_create_edges(row['sentence']), axis=1))
...
# data1= pd.read_csv('data/'+'train'+'_original.csv')
# data2= pd.read_csv('data/'+'train'+'_vi.csv')

# data=data1.merge(data2, how='inner', left_index=True, right_index=True)
# data=data[['sentence_vi', 'relationship']]
# data=data.rename(columns={'sentence_vi': 'sentence'})

# Đứa trẻ được bọc cẩn_thận và buộc vào cái nôi bằng một sợi dây
...
# partition=[6000, 8000]
# sentences, e1_distance, e2_distance, grammar, shortest_path=get_feature(data[partition[0]: partition[1]])
# label=list(data['relationship'][partition[0]: partition[1]])
...
#     pass
# else:
#     sentences.extend(features['sentences'])
#     e1_distance.extend(features['e1_distance'])
#     e2_distance.extend(features['e2_distance'])
#     grammar.extend(features['grammar'])
#     shortest_path.extend(features['shortest_path'])
#     label.extend(features['label'])

# with open('data/'+'train'+'_features.json', 'w') as w:
...
# with open('data/'+'train'+'_features.json', 'r') as r:
#                 features=json.load(r)
# len(features['sentences'])

# with open('data/'+'train'+'_features.json', 'r') as r:
#                 features=json.load(r)
# sentences=features['sentences']
# e1_distance=features['e1_distance']
# e2_distance=features['e2_distance']
...
#             data = pd.read_csv('data/'+type+'_vi.csv')
